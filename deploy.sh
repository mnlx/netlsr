...
GOOS=linux GOARCH=amd64 go build -o "build/$SERVER_BIN" ./cmd/netlsrd

echo "Building $CLIENT_BIN for linux/amd64..."
GOOS=linux GOARCH=amd64 go build -o "build/$CLIENT_BIN" ./cmd/netlsrc

# Deploy server binary and config to server host
echo "Deploying server to $SERVER_HOST..."
rsync -avz --progress "build/$SERVER_BIN" "${USER}@${SERVER_HOST}:~/"
...
rsync -avz --progress "build/$CLIENT_BIN" "${USER}@${CLIENT_HOST}:~/"
...
ssh "${USER}@${SERVER_HOST}" "nohup sudo /home/${USER}/$SERVER_BIN -config config.yaml > netlsr.log 2>&1 &"
...
ssh "${USER}@${CLIENT_HOST}" "nohup sudo /home/${USER}/$CLIENT_BIN -config config.yaml > netlsr.log 2>&1 &"
